...
def move_model_to_device(model: TabPFNClassifier, device: str) -> TabPFNClassifier:
    ...
    ``model.to(device)`` delegates to ``estimator_to_device()`` which moves
    model weights and sets ``device`` / ``devices_`` attributes, but does
    **not** move the KV-cache (``executor_.kv_caches``).  That must be
